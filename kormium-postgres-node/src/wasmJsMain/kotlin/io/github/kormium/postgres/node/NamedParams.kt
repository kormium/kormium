...
/** A SQL string with its `:name` placeholders rewritten to `$N`, plus the names in order. */
internal class ParsedSql(val sql: String, val names: List<String>)

/** Rewrites korm's `:name` placeholders to Postgres positional `$1, $2, …`, names in order. */
