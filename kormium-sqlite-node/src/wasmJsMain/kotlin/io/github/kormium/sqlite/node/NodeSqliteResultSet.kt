...
/**
 * Adapts one better-sqlite3 result row (a positional JS array, derived from the row object's
 * values in select order) to korm's [ResultSet]. The executor builds one per row, so [next] is
 * always false. Non-native korm types are stored as TEXT, so values are read through [cellText]
 * and parsed from text — the same path as the JDBC SQLite backend. Indexes are 0-based.
 */
internal class NodeSqliteResultSet(
    ...
    override fun getBytes(columnIndex: Int): ByteArray? =
        throw UnsupportedOperationException("kormium-sqlite-node does not support blob reads yet")
