...
/**
 * Rewrites korm's `:name` placeholders to SQLite's positional `?` markers, recording the names in
 * order so values bind by index. Quoted/commented sections are left untouched. (SQLite also accepts
 * `:name` natively, but positional `?` keeps binding index-based and uniform with the other engines.)
 */
