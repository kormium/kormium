 ...
 * It pins that the `transaction` member on [BackendDatabase] shadows the core extension, so the
 * locking DSL is reachable on a MySQL-typed handle and not on a portable one.
 */
@Suppress("unused")
class MySqlRowLockingGatingTest {

    private fun claimBatch(db: BackendDatabase<MyGateCatalog, MySqlBackend>): List<MyGateRow> = db.transaction {
        MyGateJobs.find {
